...
: Gtk::VBox (false, 4)
{
    ...
    canvas->signal_key_press_event ()
        .connect (bind (slot (*this, &Primitives::keyPress), canvas));
    ...
    spin_adj->signal_value_changed ()
        .connect (bind (slot (*this, &Primitives::zoomChanged),
                        canvas,
                        spin_adj));

    setupDivisions (*canvas->root ());
    setupRectangles (*canvas->root ());
    setupEllipses (*canvas->root ());
    setupImages (*canvas->root ());
    setupTexts (*canvas->root ());
    setupLines (*canvas->root ());
    setupPolygons (*canvas->root ());
    setupWidgets (*canvas->root ());
}


void
Primitives::zoomChanged (Gnome::Canvas::Canvas* canvas,
                         Gtk::Adjustment* adj)
{
...
bool
Primitives::keyPress (GdkEventKey *event, Gnome::Canvas::Canvas* canvas)
{
    int x, y;
...
bool
Primitives::itemEvent (GdkEvent* event, Gnome::Canvas::Item* item)
{
    static double x, y;
    ...
    case GDK_BUTTON_PRESS:
        ...
        case 1:
            if (event->button.state & GDK_SHIFT_MASK) {
                // gtk_object_destroy (GTK_OBJECT (item)); FIXME
            }  else {
                x = item_x;
                y = item_y;
...
Primitives::setupDivisions (Gnome::Canvas::Group& root)
{
    ...
    setupHeading (*group, "Rectangles", 0);
    setupHeading (*group, "Ellipses", 1);
    setupHeading (*group, "Texts", 2);
    setupHeading (*group, "Images", 3);
    setupHeading (*group, "Lines", 4);
    setupHeading (*group, "Curves", 5);
    setupHeading (*group, "Arcs", 6);
    setupHeading (*group, "Polygons", 7);
    setupHeading (*group, "Widges", 8);
}
...
void
Primitives::setupRectangles (Gnome::Canvas::Group& root)
{
    ...
    rect->signal_event ()
        .connect (bind (slot (*this, &Primitives::itemEvent), rect));
...
void
Primitives::setupEllipses (Gnome::Canvas::Group& root)
{
    ...
    ellipse->signal_event ()
        .connect (bind (slot (*this, &Primitives::itemEvent), ellipse));
...
Gnome::Canvas::Group*
Primitives::makeAnchor (Gnome::Canvas::Group& root, double x, double y)
{
    Gnome::Canvas::Group* group = manage (new Gnome::Canvas::Group (root));
    group->property_x () = x;
    group->property_y () = y;
...
void
Primitives::setupTexts (Gnome::Canvas::Group& root)
{
    Gnome::Canvas::Text* text;

    if (root.get_canvas ()->property_aa () == true) {
        text = manage (new Gnome::Canvas::Text (*makeAnchor (root, 
                                                             420.0, 20.0)));
...
void
Primitives::plantFlower (Gnome::Canvas::Group& root,
                         double x, double y)
{
    ...
    if (image) {
        ...
        pixbuf->signal_event ()
            .connect (bind (slot (*this, &Primitives::itemEvent), pixbuf));
    }
}


void
Primitives::setupImages (Gnome::Canvas::Group& root)
{
    Glib::RefPtr<Gdk::Pixbuf> image 
    ...
    plantFlower (root,  20.0, 170.0);
    plantFlower (root, 180.0, 170.0);
    plantFlower (root,  20.0, 280.0);
    plantFlower (root, 180.0, 280.0);
}
...
void
Primitives::polishDiamond (Gnome::Canvas::Group& root)
{
    Gnome::Canvas::Group* group = manage (new Gnome::Canvas::Group (root));
...
Primitives::makeHilbert (Gnome::Canvas::Group& root)
{
    ...
    if (root.get_canvas ()->property_aa () == true) {
        ...
        line->signal_event ()
            .connect (bind (slot (*this, &Primitives::itemEvent), line));
    } else {
        Gnome::Canvas::Line* line = manage (new Gnome::Canvas::Line (root));
        line->property_points ().set_value (points);
...
void
Primitives::setupLines (Gnome::Canvas::Group& root)
{
    polishDiamond (root);
    makeHilbert (root);
...
void
Primitives::setupPolygons (Gnome::Canvas::Group& root)
{
    Gnome::Canvas::Points points;
    ...
    if (root.get_canvas ()->property_aa () == true) {
        ...
        polygon->signal_event ()
            .connect (bind (slot (*this, &Primitives::itemEvent), polygon));
    } else {
        Gnome::Canvas::Polygon* polygon 
            = manage (new Gnome::Canvas::Polygon (root));
        polygon->property_points ().set_value (points);
...
void
Primitives::setupWidgets (Gnome::Canvas::Group& root)
{
...
//     widget->property_y () = 330.0;
    ...
    widget->signal_event ()
        .connect (bind (slot (*this, &Primitives::itemEvent), widget));
}
